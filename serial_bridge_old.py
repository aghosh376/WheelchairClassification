...
Command mapping (matches commands produced by live_predict.py / live_predict_wrapper.py):
  "STOP" / "STOP (Low Confidence)" / "UNKNOWN"  →  "0"  (LED off)
  "FORWARD"                                      →  "1"  (slow blink)
  "REVERSE"                                      →  "2"  (fast blink)

Add new wheelchair labels to COMMAND_MAP when motor control is introduced.
"""

import time

# ── Label → serial command mapping ─────────────────────────────────────────
# Entries from live_predict.py output are listed first.
# Generic aliases below are useful for manual testing.
# ── Add future wheelchair direction labels here ──────────────────────────
COMMAND_MAP = {
    # live_predict.py / live_predict_wrapper.py outputs
    "STOP":                  "0",
    ...
    # Generic aliases for manual / testing use
    "stop":    "0",
    "off":     "0",
    "neutral": "0",
    "forward": "1",
    "left":    "1",   # placeholder — remap when left/right classes are trained
    "right":   "2",   # placeholder
    "fast":    "2",
    "reverse": "2",

    # Raw commands pass through
    "0": "0",
    "1": "1",
    "2": "2",
}
...
class ESP32SerialBridge:
    ...
    def connect(self):
        ...
        if not self.port:
            raise ValueError(
                ...
                "Find available ports with:  ls /dev/cu.*"
            )
    ...
    def send_command(self, command_or_label: str) -> str:
        ...
        Accepts a high-level label ("FORWARD", "STOP", …) or raw command
        ("0", "1", "2").  Unknown labels default safely to "0" (stop).

        Returns the raw command string that was sent / would be sent.
        """
        raw_cmd = COMMAND_MAP.get(command_or_label)
        ...
        if self.mock:
            print(f"[MockBridge] MOCK SEND TO ESP32: {raw_cmd}")
        else:
